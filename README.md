...
Hermes34는 물리적으로 **`wr_content_t` 한 테이블**을 사용합니다. 고객·프로젝트·사이트는 같은 행을 `module_idx`로 구분한 논리 모듈입니다. 원본 서비스 시트의 한 행은 세 모듈 행으로 나누어 적재하며, 공용 필드에 저장한 값은 `content_raw`에 다시 넣지 않습니다.
...
- 고객 → 프로젝트와 프로젝트 → 사이트 연결은 자식 행의 `parent_idx`를 사용합니다.
- 프로젝트에는 운영상태와 기간을 `status`, `date_start`, `date_end`로 저장합니다. 종료일 원본이 없으면 `date_end=2999-12-31`을 사용합니다.
- 사이트에는 `구분 (개발/운영)`을 `type`으로 저장하고, 도메인주소와 호스팅주소를 `url`에 쉼표로 구분해 저장합니다.
