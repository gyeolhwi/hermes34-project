...
원본 서비스 시트 한 행은 고객·프로젝트·서비스 3개 모듈 행으로 정리합니다.
...
| F~G | 도메인주소, 호스팅주소 | 서비스 `url`: `도메인주소, 호스팅주소` | `content_raw` |
| H~I | 호스팅 접속정보 | 서비스 `content_raw.access.hosting` | — |
| J~K | 관리자 접속정보 | 서비스 `content_raw.access.admin` | — |
| L~N | DB 접속정보 | 서비스 `content_raw.access.database` | — |
| O~P | 고객연락처, 고객이메일 | 서비스 원문; 고객 확정 후 고객 `content_raw.contact[]` | — |
| Q~S | 도메인등록업체, 호스팅사, 프레임워크 | 서비스 `tags` | `content_raw` |
| T~U | 검수메모, 비고 | 서비스 또는 프로젝트 `content` | `content_raw` |
| V | 작업자 | 서비스 `receiver_idx` (member 대조 후) | `content_raw` |
| W | 개설일 | 프로젝트 `date_start` | `content_raw` |

서비스 행은 접속·DB 정보 등 암호화 보관 대상이 있으므로 `is_hidden=1`을 설정합니다.
